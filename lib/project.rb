class Project
  ...
  def get_volunteers
    ...
    volunteers.each do |volunteer|
      ...
      list_volunteers.push(Volunteer.new({volunteer_name: volunteer_name, project_id: project_id, id: id}))
    end
    list_volunteers
  end
  ...
  def update(attributes)
    @project_name = attributes.fetch(:project_name)
    DB.exec("UPDATE projects SET project_name = '#{@project_name}' WHERE id = #{self.id};")
  end
